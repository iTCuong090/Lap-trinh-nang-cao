...
public class Main {
    public static void main(String[] args) {
        ...
        // Regex "[^a-z\\s]" nghĩa là: giữ lại chữ cái (a-z) và khoảng trắng (\\s), xóa
        // mọi thứ khác ví dụ như dấu câu, emoji, vv.
        String normalizedText = text.toLowerCase().replaceAll("[^a-z\\s]", "");

        // Tách chuỗi thành mảng các từ, phân cách bằng một khoảng trắng.
        // "\\s+" bao gồm "\s" có nghĩa là bất kì loại kí tự khoảng trắng nào (tab,
        // xuống dòng, vv), + có nghĩa là từ 1 trở lên, \ là kí tự thoát String.
        ...
        for (String word : words) {
            ...
            if (!wordCountMap.containsKey(word)) {
                // Nếu từ chưa có trong Map thì thêm chữ đó vào và gán giá trị bằng 1 (đã xuất
                // hiện 1 lần)
        ...
        for (Map.Entry<String, Integer> entry : wordCountMap.entrySet()) { // Map.Entry<> là kiểu, là lớp của các
                                                                           // key-value được lưu trong map.
            String word = entry.getKey(); // Phương thức .entrySet() của lớp Map trả về mảng của các đối tượng
                                          // map.entry.
